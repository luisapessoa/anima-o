...
function TopVideo({ src, start, end, speed, w, h, shiftY }) {
...
function FullVideoPlain({ src, start, end, speed, shiftY }) {
  const [ready, setReady] = React.useState(false);
  const readyRef = React.useRef(false);
  React.useEffect(() => {
    const t = setTimeout(() => { if (!readyRef.current) { readyRef.current = true; setReady(true); } }, 1200);
...
function fmt(text, color, weight, key) {
  ...
  return React.createElement('span', { key },
    parts.map((p, i) => i % 2 === 1
      ? React.createElement('span', { key: i, style: { color, fontWeight: weight || 700 } }, p)
  ...
  return React.createElement('span', { key },
    parts.map((part, i) => i % 2 === 1
      ? React.createElement('span', { key: i, style: {
          backgroundImage: `linear-gradient(${hi}, ${hi})`, backgroundRepeat: 'no-repeat',
          backgroundPosition: 'left center', backgroundSize: `${p * 100}% 76%`,
          color: tc, fontWeight: 700, padding: '2px 8px',
          boxDecorationBreak: 'clone', WebkitBoxDecorationBreak: 'clone' } }, part)
...
function fmtHi(text, hi, tc, key) {
  ...
  return React.createElement('span', { key },
    parts.map((p, i) => i % 2 === 1
      ? React.createElement('span', { key: i, style: { background: hi, color: tc, fontWeight: 700,
          padding: '2px 8px', boxDecorationBreak: 'clone', WebkitBoxDecorationBreak: 'clone' } }, p)
      : React.createElement(React.Fragment, { key: i }, p)));
}
  ...
  return (
    <div style={{ ...shell, background: BLACK }}>
      ...
      <div style={{ position: 'absolute', left: boxL + boxW - sq - 160, top: boxTop - sq * 0.55,
        width: sq, height: sq, background: SKY, opacity: tile,
        transform: `scale(${0.85 + 0.15 * tile})`, transformOrigin: 'center',
        display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <svg viewBox="0 -23 412.8 412" style={{ width: sq * 0.52, height: sq * 0.52 }}>
          <path fill={NAVY} d={CHAT_D} /></svg>
      </div>
      <div style={{ position: 'absolute', left: boxL + 66, right: boxL + 66, top: boxTop + 150 }}>
        {(sc.head || []).map((ln, i) => (
          <div key={i} style={{ ...rise(lt, 0.85 + i * 0.1, 16), color: WHITE, fontWeight: 700,
            fontSize: 74, lineHeight: 1.16, letterSpacing: '-0.02em' }}>{ln}</div>
        ))}
      </div>
    </div>
  );
...
function LightList() {
  ...
  return (
    <div style={{ ...shell, background: GREY, display: 'flex', flexDirection: 'column',
      justifyContent: 'flex-start', alignItems: 'center', padding: '520px 0 0 0', ...groupIn(lt, 'up') }}>
      {RUNTIME.showLogo ? <Logo variant="dark" lt={lt} /> : null}
      <div style={{ width: 'fit-content' }}>
      <div>
        {(sc.head || []).map((ln, i) => (
          <div key={i} style={{ ...rise(lt, 0.2 + i * 0.08, 18), color: NAVY, fontWeight: 700,
            fontSize: 76, lineHeight: 1.12, letterSpacing: '-0.02em', whiteSpace: 'nowrap' }}>{ln}</div>
        ))}
      </div>
      <div style={{ marginTop: 56 }}>
        {(sc.body || []).map((ln, i) => (
          <div key={i} style={{ ...rise(lt, 0.55 + i * 0.08, 14), color: NAVY, fontWeight: 400,
            fontSize: 54, lineHeight: 1.28, whiteSpace: 'nowrap' }}>{ln}</div>
        ))}
      </div>
      <div style={{ display: 'flex', gap: 34, marginTop: 52, marginLeft: 40 }}>
        <div style={{ width: 7, background: SKY, borderRadius: 4,
          transform: `scaleY(${bar})`, transformOrigin: 'top' }} />
        <div>
          {(sc.quote || []).map((ln, i) => (
            <div key={i} style={{ ...rise(lt, 1.0 + i * 0.08, 12), color: NAVY, fontWeight: 400,
              fontSize: 54, lineHeight: 1.28, whiteSpace: 'nowrap' }}>{ln}</div>
          ))}
        </div>
      </div>
      <div style={{ marginTop: 56 }}>
        {(sc.strong || []).map((ln, i) => (
          <div key={i} style={{ ...rise(lt, 1.45 + i * 0.08, 12), color: NAVY, fontWeight: 700,
            fontSize: 54, lineHeight: 1.24, whiteSpace: 'nowrap' }}>{ln}</div>
        ))}
      </div>
      </div>
    </div>
...
function SkyBox() {
  ...
  return (
    <div style={{ ...shell, background: NAVY }}>
      ...
      <div style={{ position: 'absolute', left: 150, top: 470, width: 820, ...groupIn(lt, 'right') }}>
        {(sc.head || []).map((ln, i) => (
          <div key={i} style={{ ...rise(lt, 0.2 + i * 0.08, 16), color: SKY, fontWeight: 700,
            fontSize: 68, lineHeight: 1.12, letterSpacing: '-0.02em' }}>{ln}</div>
        ))}
      </div>
      <svg width={W} height={H} style={{ position: 'absolute', inset: 0 }}>
        <path d="M 165 690 V 785 H 300 M 300 690 V 884" fill="none" stroke={SKY} strokeWidth="4"
          strokeLinecap="round" strokeLinejoin="round" pathLength="1"
          strokeDasharray="1" strokeDashoffset={1 - draw} />
      </svg>
      <div style={{ position: 'absolute', left: 330, right: 100, top: 690 }}>
        {(sc.body || []).map((ln, i) => (
          <div key={i} style={{ ...rise(lt, 0.75 + i * 0.09, 14), color: WHITE, fontWeight: 400,
            fontSize: 52, lineHeight: 1.24 }}>{ln}</div>
        ))}
      </div>
      <div style={{ position: 'absolute', left: 120, right: 120, top: 1000, background: SKY,
        padding: '52px 56px', textAlign: 'center', opacity: box,
        transform: `translateY(${(1 - box) * 34}px)` }}>
        {(sc.box || []).map((ln, i) => (
          <div key={i} style={{ ...rise(lt, 1.1 + i * 0.07, 10), color: NAVY, fontWeight: 400,
            fontSize: 48, lineHeight: 1.26 }}>{fmt(ln, NAVY, 700, i)}</div>
        ))}
      </div>
      <img src="assets/real/car-polo.webp" alt="Volkswagen Polo"
        style={{ position: 'absolute', left: '50%', bottom: 90, width: 1060, height: 'auto',
...
function Factors() {
  ...
  const splitY = 830;
  const Sq = ({ d }) => (
    <span style={{ display: 'inline-block', width: 22, height: 22, background: SKY,
      marginRight: 20, marginTop: 30, flex: '0 0 auto', opacity: ease(lt, d, 0.4) }} />
  );
  return (
      ...
      <div style={{ position: 'absolute', left: 150, right: 120, top: splitY + 160, ...groupIn(lt, 'up') }}>
        {(sc.head || []).map((ln, i) => (
          <div key={i} style={{ ...rise(lt, 0.2 + i * 0.08, 16), color: NAVY, fontWeight: 700,
            fontSize: 56, lineHeight: 1.14, letterSpacing: '-0.02em', whiteSpace: 'nowrap' }}>{ln}</div>
        ))}
        <div style={{ display: 'flex', marginTop: 58 }}>
          <div style={{ flex: 1 }}>
            {(sc.colA || []).map((ln, i) => (
              <div key={i} style={{ ...rise(lt, 0.7 + i * 0.07, 12), color: NAVY, fontWeight: 400,
                fontSize: 52, lineHeight: 1.2, display: 'flex', alignItems: 'flex-start' }}>
                {i === 0 ? <Sq d={0.7} /> : <span style={{ width: 42, flex: '0 0 auto' }} />}
                <span>{ln}</span></div>
            ))}
          </div>
          <div style={{ flex: 1 }}>
            {(sc.colB || []).map((ln, i) => (
              <div key={i} style={{ ...rise(lt, 0.85 + i * 0.07, 12), color: NAVY, fontWeight: 400,
                fontSize: 52, lineHeight: 1.2, display: 'flex', alignItems: 'flex-start' }}>
                {i === 0 ? <Sq d={0.85} /> : <span style={{ width: 42, flex: '0 0 auto' }} />}
                <span>{ln}</span></div>
            ))}
          </div>
        </div>
        <div style={{ marginTop: 44 }}>
          {(sc.wide || []).map((ln, i) => (
            <div key={i} style={{ ...rise(lt, 1.05 + i * 0.07, 12), color: NAVY, fontWeight: 400,
              fontSize: 52, lineHeight: 1.2, display: 'flex', alignItems: 'flex-start' }}>
              {i === 0 ? <Sq d={1.05} /> : <span style={{ width: 42, flex: '0 0 auto' }} />}
              <span>{ln}</span></div>
          ))}
        </div>
        <div style={{ marginTop: 48 }}>
          {(sc.strong || []).map((ln, i) => (
            <div key={i} style={{ ...rise(lt, 1.35 + i * 0.08, 12), color: NAVY, fontWeight: 700,
              fontSize: 54, lineHeight: 1.3 }}>{fmtHiAnim(ln, SKY, NAVY, ease(lt, 1.55 + i * 0.45, 0.55), i)}</div>
          ))}
        </div>
      </div>
    </div>
...
function Included() {
  ...
  const listTop = 780, itemGap = 124, lineX = 163, sqX = 150, textX = 222;
  const elbowY = listTop + items.length * itemGap + 24;
  return (
    <div style={{ ...shell, background: NAVY }}>
      {RUNTIME.showLogo ? <Logo variant="white" lt={lt} /> : null}
      <div style={{ position: 'absolute', inset: 0, transform: 'translateX(-40px)' }}>
      <div style={{ position: 'absolute', left: 150, right: 130, top: 420, ...groupIn(lt, 'up') }}>
        {(sc.head || []).map((ln, i) => (
          <div key={i} style={{ ...rise(lt, 0.2 + i * 0.07, 14), color: SKY, fontWeight: 700,
            fontSize: 54, lineHeight: 1.18, letterSpacing: '-0.01em' }}>{ln}</div>
        ))}
      </div>
      <svg width={W} height={H} style={{ position: 'absolute', inset: 0 }}>
        <path d={`M ${lineX} ${listTop + 8} V ${elbowY - 12} H 400`} fill="none" stroke={SKY} strokeWidth="4"
          strokeLinecap="round" strokeLinejoin="round" pathLength="1"
          strokeDasharray="1" strokeDashoffset={1 - draw} />
      </svg>
      {items.map((it, i) => {
        ...
        return (
          <React.Fragment key={i}>
            <div style={{ position: 'absolute', left: sqX, top: y - 4, width: 26, height: 26,
              background: SKY, opacity: p, transform: `scale(${0.6 + 0.4 * p})` }} />
            <div style={{ position: 'absolute', left: textX, top: y - 22, ...rise(lt, 0.55 + i * 0.12, 12),
              color: WHITE, fontWeight: 500, fontSize: 62, lineHeight: 1 }}>{it}</div>
          </React.Fragment>
        );
      })}
      <div style={{ position: 'absolute', right: 70, top: elbowY - 46 }}>
        {(sc.tail || []).map((ln, i) => (
          <div key={i} style={{ ...rise(lt, 1.6 + i * 0.08, 12), color: WHITE, fontWeight: 400,
            fontSize: 56, lineHeight: 1.26, whiteSpace: 'nowrap' }}>{ln}</div>
        ))}
      </div>
      </div>
      <div style={{ position: 'absolute', left: 0, right: 0, top: 1560, textAlign: 'center',
        ...rise(lt, 2.0, 10) }}>
        <span style={{ color: WHITE, fontWeight: 400, fontSize: 40, opacity: 0.85 }}>{sc.note}</span>
      </div>
    </div>
  );
}
    ...
    <div style={{ ...shell, background: BLACK }}>
      ...
      <div style={{ position: 'absolute', left: 150, right: 120, top: 1310, ...groupIn(lt, 'up') }}>
        ...
        <div style={{ marginTop: 46 }}>
          {(sc.body || []).map((ln, i) => (
            <div key={i} style={{ ...rise(lt, 0.9 + i * 0.08, 14), color: WHITE, fontWeight: 400,
              fontSize: 46, lineHeight: 1.34, whiteSpace: 'nowrap' }}>{fmt(ln, WHITE, 700, i)}</div>
          ))}
        </div>
      </div>
    </div>
...
function LogoFrame() {
  ...
  const FR = { l: 90, r: 90, t: 720, b: 600 };
  ...
  return (
    <div style={{ ...shell, background: BLACK }}>
      ...
      <div style={{ position: 'absolute', left: FR.l, right: FR.r, top: FR.t + fh / 2,
        transform: 'translateY(-50%)', textAlign: 'center', padding: '0 40px' }}>
        {(sc.body || []).map((ln, i) => (
          <div key={i} style={{ ...rise(lt, 1.1 + i * 0.1, 14), color: WHITE, fontWeight: 700,
            fontSize: 60, lineHeight: 1.3, letterSpacing: '-0.01em' }}>{fmtU(ln, SKY, i)}</div>
        ))}
      </div>
    </div>
  );
...
/* ── root ────────────────────────────────────────────────────── */
const LAYOUTS = {
  question: Question, lightlist: LightList, skybox: SkyBox, factors: Factors,
  included: Included, line: Line, logoframe: LogoFrame,
};
...
function RealVideo() {
  ...
  const children = {};
  scenes.forEach((sc) => { children[sc.name] = LAYOUTS[sc.layout] || LightList; });
  return (
    <React.Fragment>

...
function rich(text, accent, key) {
  ...
  return React.createElement('span', { key },
    parts.map((p, i) => i % 2 === 1
      ? React.createElement('b', { key: i, style: { fontWeight: 700, color: accent || 'inherit' } }, p)
...
function Counter({ to, lt, delay, dur }) {
  ...
  return (
    <span style={{ display: 'inline-block', position: 'relative', fontVariantNumeric: 'tabular-nums', color: 'inherit', fontWeight: 700 }}>
      <span style={{ visibility: 'hidden' }}>{res}</span>
      <span style={{ position: 'absolute', left: 0, top: 0 }}>{cur}</span>
    </span>
  );
}
...
function Bracket() {
  const s = useScene(); const lt = s.localTime;
  const draw = ease(lt, 0.9, 1.2);
  const hs = { color: WHITE, fontWeight: 700, fontSize: 66, lineHeight: 1.16, letterSpacing: '-0.02em', whiteSpace: 'nowrap' };
  ...
  return (
    <div style={{ ...shell }}>
      ...
      <div style={{ position: 'absolute', left: 108, right: 100, top: 420 }}>
        ...
        <div style={{ ...rise(lt, 0.33, 18), ...hs }}>ou <span style={num}><Counter to={1500} lt={lt} delay={0.7} /></span> km por mês é</div>
        <div style={{ ...rise(lt, 0.41, 18), ...hs }}>pouco para assinar um</div>
        <div style={{ ...rise(lt, 0.49, 18), ...hs }}>carro?</div>
      </div>
      <svg viewBox="0 0 1080 1920" preserveAspectRatio="none"
        style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }}>
        <path d="M 782 512 H 900 V 1400" fill="none" stroke={MINT} strokeWidth="3"
          strokeLinecap="round" strokeLinejoin="round"
          pathLength="1" strokeDasharray="1" strokeDashoffset={1 - draw} />
      </svg>
      <div style={{ position: 'absolute', right: 110, left: 400, top: 1440, textAlign: 'right' }}>
        <Lines list={["Talvez você esteja", "fazendo essa conta", "do jeito errado!"]}
          lt={lt} delay={1.5} step={0.1}
          style={{ color: WHITE, fontWeight: 400, fontSize: 64, lineHeight: 1.22, whiteSpace: 'nowrap' }} />
      </div>
    </div>
  );
}
    ...
    <div style={{ ...shell }}>
      ...
      <div style={{ position: 'absolute', left: 108, right: 90, top: 1140 }}>
        <div style={{ width: 210, height: 8, background: YEL, borderRadius: 4,
          transformOrigin: 'left', transform: `scaleX(${ease(lt, 0.25, 0.6)})`, marginBottom: 66 }} />
        <Lines list={sc.p1} lt={lt} delay={0.5} step={0.08}
          style={{ color: WHITE, fontWeight: 400, fontSize: 62, lineHeight: 1.28 }} />
        <div style={{ height: 52 }} />
        <Lines list={sc.p2} lt={lt} delay={0.95} step={0.08}
          style={{ color: WHITE, fontWeight: 700, fontSize: 68, lineHeight: 1.18, letterSpacing: '-0.01em' }} />
      </div>
    </div>
  );
}
...
function Roads() {
  ...
  return (
    <div style={{ ...shell, background: CARD }}>
      ...
      <img src="assets/roads-bottom.png" alt=""
        style={{ position: 'absolute', bottom: 0, left: 0, width: 1080, height: 'auto',
                 clipPath: clip(wipeB), WebkitClipPath: clip(wipeB),
      ...
      <div style={{ position: 'absolute', left: 150, right: 150, top: 620 }}>
        <Lines list={sc.p1} lt={lt} delay={0.6} step={0.08}
          style={{ color: INK, fontWeight: 400, fontSize: 62, lineHeight: 1.24 }} />
      </div>
      <div style={{ position: 'absolute', left: 0, top: 1138, width: 380, height: 3, background: MINT,
        transformOrigin: 'left', transform: `scaleX(${lineW})` }} />
      <div style={{ position: 'absolute', left: 470, right: 70, top: 1100, textAlign: 'left' }}>
        <Lines list={sc.p2} lt={lt} delay={1.05} step={0.09}
          style={{ color: INK, fontWeight: 400, fontSize: 62, lineHeight: 1.24, whiteSpace: 'nowrap' }} />
      </div>
    </div>
  );
}
...
function MintCard() {
  const s = useScene(); const lt = s.localTime;
  const cardTop = 560, cardH = 840;
  const ts = { color: INK, fontWeight: 400, fontSize: 58, lineHeight: 1.3 };
  ...
  return (
    <div style={{ ...shell }}>
      ...
      <div style={{ position: 'absolute', left: 130, right: 130, top: cardTop, height: cardH,
        ...
        <div style={{ position: 'absolute', left: 78, right: 70, top: 0, bottom: 0,
          display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          <div style={{ ...rise(lt, 0.5, 16), ...ts }}>
            <b style={b}>Ou seja:</b> para muitos perfis,<br />uma franquia de <b style={b}>1.000 ou</b><br /><b style={b}>1.500 km mensais</b> atende<br />muito bem a rotina.
          </div>
          <div style={{ height: 48 }} />
          <div style={{ ...rise(lt, 0.85, 16), ...ts }}>
            Principalmente para quem<br />usa o carro no dia a dia, em<br />trajetos urbanos e<br />deslocamentos comuns.
          </div>
        </div>
      </div>
    </div>
    ...
    <div style={{ ...shell }}>
      ...
      <div style={{ position: 'absolute', left: 108, right: 90, top: 380 }}>
        <Lines list={sc.title} lt={lt} delay={0.3} step={0.1}
          style={{ color: YEL, fontWeight: 700, fontSize: 84, lineHeight: 1.12, letterSpacing: '-0.02em' }} />
        <div style={{ height: 64 }} />
        <Lines list={sc.body} lt={lt} delay={0.7} step={0.08}
          style={{ color: WHITE, fontWeight: 400, fontSize: 58, lineHeight: 1.32 }} />
      </div>
    </div>
  );
}
...
function Checklist() {
  ...
  const RAIL = 290, DOT = 20, FS = 58, LH = 1.2;
  return (
    <div style={{ ...shell, background: TEAL }}>
      {RT.showLogo ? <Logo /> : null}
      <div style={{ position: 'absolute', left: 130, right: 90, top: 540 }}>
        <Lines list={sc.title} lt={lt} delay={0.3} step={0.1}
          style={{ color: WHITE, fontWeight: 700, fontSize: 84, lineHeight: 1.14, letterSpacing: '-0.02em' }} />
      </div>
      <div style={{ position: 'absolute', left: 0, right: 90, top: 960,
        ...
        {items.map((lines, i) => {
          ...
          return (
            <div key={i} style={{ display: 'flex', alignItems: 'flex-start' }}>
              <div style={{ position: 'relative', width: RAIL, flex: '0 0 auto', alignSelf: 'stretch' }}>
                <div style={{ position: 'absolute', left: 0, top: cy - 1.5, width: RAIL - DOT - 56, height: 3,
                  background: YEL, transformOrigin: 'left', transform: `scaleX(${ease(lt, d, 0.55)})` }} />
                <div style={{ position: 'absolute', left: RAIL - DOT - 66, top: cy - DOT / 2, width: DOT, height: DOT,
                  borderRadius: '50%', background: YEL, opacity: ease(lt, d + 0.4, 0.3) }} />
              </div>
              <div style={{ ...rise(lt, d + 0.15, 12), color: WHITE, fontWeight: 400, fontSize: FS, lineHeight: LH }}>
    ...
    <div style={{ ...shell }}>
      ...
      <div style={{ position: 'absolute', left: 90, right: 90, top: 480, textAlign: 'center' }}>
        <Lines list={sc.top} lt={lt} delay={0.35} step={0.09}
          style={{ color: WHITE, fontWeight: 400, fontSize: 66, lineHeight: 1.24 }} />
      </div>
      <div style={{ position: 'absolute', left: 540, top: 900, width: 3, height: 240, background: MINT,
        transformOrigin: 'top', transform: `translateX(-1.5px) scaleY(${conn})` }} />
      <div style={{ position: 'absolute', left: 70, right: 70, top: 1140, minHeight: 380,
        background: MINT, opacity: box, transform: `translateY(${(1 - box) * 24}px)`,
        display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '56px 48px' }}>
        <div style={{ textAlign: 'center' }}>
          <Lines list={sc.box} lt={lt} delay={1.45} step={0.08}
            style={{ color: INK, fontWeight: 700, fontSize: 58, lineHeight: 1.26, letterSpacing: '-0.02em', whiteSpace: 'nowrap' }} />
        </div>
      </div>
    </div>
  );
...
/* ── root ────────────────────────────────────────────────── */
const LAYOUTS = {
  bracket: Bracket, ruled: Ruled, roads: Roads, mintcard: MintCard,
  titlelead: TitleLead, checklist: Checklist, closer: Closer,
};
...
function SudesteVideo4() {
  ...
  const children = {};
  scenes.forEach((sc) => { children[sc.name] = LAYOUTS[sc.layout] || Bracket; });
  return (
    <React.Fragment>

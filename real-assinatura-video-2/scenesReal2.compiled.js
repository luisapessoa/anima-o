...
function groupIn(lt, dir, d) {
  ...
  const off = 1 - p;
  let t = '';
  if (dir === 'up') t = `translateY(${off * 60}px)`;else if (dir === 'down') t = `translateY(${off * -60}px)`;else if (dir === 'left') t = `translateX(${off * 60}px)`;else if (dir === 'right') t = `translateX(${off * -60}px)`;
  return {
    opacity: p,
    transform: t
  };
...
function WhyMatters() {
  ...
  }, (sc.tag || []).map((ln, i) => /*#__PURE__*/React.createElement("div", {
    key: i,
    style: {
      ...rise(lt, 1.2 + i * 0.09, 14),
      color: WHITE,
      fontWeight: 700,
      fontSize: 52,
      lineHeight: 1.22,
...
function TickList() {
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: {
      ...
      left: 150,
      right: 150,
      top: listTop + items.length * rowH + 100,
      textAlign: 'right'
    }
  }, (sc.tail || []).map((ln, i) => /*#__PURE__*/React.createElement("div", {
    key: i,
    style: {
      ...rise(lt, 2.5 + i * 0.12, 16),
      color: NAVY,
      fontWeight: 700,
      fontSize: 50,
      lineHeight: 1.16,
...
function VLinePara() {
  ...
  const lt = s.localTime;
  const sc = s.scene;
  const lines1 = sc.lines1 || [];
  const lines2 = sc.lines2 || [];
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: {
      ...
      left: 150,
      right: 120,
      top: lineTop + lineH + 90
    }
  }, lines2.map((ln, i) => /*#__PURE__*/React.createElement("div", {
    key: i,
    style: {
      ...rise(lt, 1.25 + i * 0.08, 14),
      color: WHITE,
      fontWeight: 400,
      fontSize: 46,
      lineHeight: 1.26,
...
function QuestionList() {
  ...
  const listTop = 900,
    sqX = 150,
    textX = 210;
  const lineHeightPx = 46 * 1.1,
    gap = 78;
  const offsets = [];
  let acc = 0;
  items.forEach(lines => {
    offsets.push(acc);
    acc += lines.length * lineHeightPx + gap;
  });
...
function VideoRule() {
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: {
      position: 'absolute',
      left: 150,
      top: zoneH + 55 + sc.body.length * 61 + 40,
      width: W - 300,
      height: 4,
      background: SKY,
      transform: `scaleX(${ruleBottom})`,
      transformOrigin: 'center'
    }
  }));
}
...
function RealVideo2() {
  ...
  const children = {};
  scenes.forEach(sc => {
    children[sc.name] = LAYOUTS[sc.layout] || WhyMatters;
  });
